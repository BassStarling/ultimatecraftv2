...
public class DisposableArcFurnaceEmiRecipe implements EmiRecipe {
    ...
    @Override
    public int getDisplayWidth() { return 150; }
    @Override
    ...
    public void addWidgets(WidgetHolder widgets) {
        ...
        widgets.addSlot(EmiStack.of(ModBlocks.DISPOSABLE_ARC_FURNACE.get()), 45, 10).drawBack(false);

        // 出力アイテム（グラファイト電極）
        widgets.addSlot(outputs.get(0), 78, 10).large(true).recipeContext(this);

        widgets.addText(Component.literal("スパークストーンTier7"), 0, 40, 0xFFD700, true);
        widgets.addText(Component.literal("[超高密度スパークストーン]を要求"), 0, 50, 0xFFD700, true);
    }
}